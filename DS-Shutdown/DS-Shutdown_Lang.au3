...
Global $mButtons, $mTraymenu, $mLabels, $mMessages
...
; [3] "de-DE"          -> IniFilename without .ini suffix
...
Local $aAllLanguages[$iLanguageCount][4] = [ _
		...
		["0407", "de-DE", "Deutsch - Deutsch", "de-DE"], _
		["0408", "el-GR", "Greek - Greece"], _
		["0409", "en-US", "English - United States", "en-EN"], _
		...
		["0809", "en-GB", "English - United Kingdom", "en-EN"], _
...
Func GetCurrentLanguageIniFile($OSLang = @OSLang)
	; C:\Program Files (x86)\OpenDigitalSignage.org\DS-Layout\Lang\en-EN.ini
	; C:\Program Files (x86)\OpenDigitalSignage.org\DS-Schedule\Lang\en-EN.ini
	; C:\Program Files (x86)\OpenDigitalSignage.org\DS-Shutdown\Lang\en-EN.ini
	Local $sLangDir = @ScriptDir & "\" & $sAppName
	Local $sLangDefault = $sLangDir & "\en-EN.ini"
	Local $sLangFile
	...
	For $i = 0 To $iLanguageCount - 1
		...
		; found correct langfile
		$sLangFile = $sLangDir & "\" & $aAllLanguages[$i][1] & ".ini"
		If FileExists($sLangFile) Then Return $sLangFile
	Next

	; nothing found, return default
	If Not FileExists($sLangDefault) Then
		MsgBox($MB_ICONINFORMATION, $sTitle, "Can't find my language Files :/" & @CRLF & "Exiting now.")
		Exit
	EndIf
...
Func InitMsg($sIniName, $sSection, ByRef $mArray, $iCount)
	...
	If @error Then
		MsgBox(0, $sTitle, "Reading language file failed! error=" & @error)
		Exit
	EndIf

	If $iCount <> $a[0][0] Then
		MsgBox(0, $sTitle, "Reading language file failed!")
		Exit
	EndIf
...
; ===============================================================================================================================
Func InitLanguage()
	;Local $sIniName = GetCurrentLanguageIniFile("4009")
	Local $sIniName = GetCurrentLanguageIniFile()
	; ConsoleWrite("LangFile = " & $sIniName & @CRLF)

	InitMsg($sIniName, "Buttons", $mButtons, 5)
	InitMsg($sIniName, "Traymenu", $mTraymenu, 3)
	InitMsg($sIniName, "Labels", $mLabels, 2)
	InitMsg($sIniName, "Messages", $mMessages, 2)
